...
for fig_num in range(4):
    if fig_num == 3:
        ax[fig_num].set_title(chr(948) + ' = 1.0', fontsize=FONT_SIZE*1.2)
    else:
        ax[fig_num].set_title(chr(948) + ' = 0.{}'.format(4 + 2 * fig_num), fontsize=FONT_SIZE*1.2)
    with open('output_8_{}.txt'.format(4 + 2 * fig_num)) as file:
...
plt.savefig('result_target.png', bbox_inches='tight')
plt.savefig('result_target.pdf', bbox_inches='tight')
